...
#################################################################
echo "start download the rocketmq release 4.9.4"
if [ ${version} == "4.9.4" ]
then
...
then
  ...
  then
   #Try another source
   wget https://mirrors.tuna.tsinghua.edu.cn/apache/rocketmq/5.1.0/rocketmq-all-5.1.0-bin-release.zip
   if [[ $? -ne 0 ]]
   then
     #Try another  source
     wget https://mirrors.bfsu.edu.cn/apache/rocketmq/5.1.0/rocketmq-all-5.1.0-bin-release.zip
     if [[ $? -ne 0 ]]
     then
       echo "failed to download the rocketMQ from website"
       exit 1
